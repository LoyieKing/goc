...
//   sptr to non-stack cptr/auto T* storage promotes that storage to uptr in
//   the IR pass; unsupported escapes and raw sptr returns remain hard errors.
//
    ...
    if (UO->getOpcode() == UO_AddrOf) {
      if (const auto *DRE =
        ...
        if (const auto *VD = dyn_cast<VarDecl>(DRE->getDecl())) {
          ...
          if (C != GocColor::None && C != GocColor::Auto)
            return C;
          // Address of a stack object is sptr; the address of a global/static
          // object is cptr (contract 3.1: cptr covers globals / data segment).
          return VD->hasLocalStorage() ? GocColor::SPtr : GocColor::CPtr;
        }
      }
    }
  }
